...
var payCentres = { 
    mastercard: {x: w / 3.65, y: h / 2.3},
		visa: {x: w / 3.65, y: h / 1.8},
		diners: {x: w / 1.15, y: h / 1.9},
		amex: {x: w / 1.12, y: h  / 3.2 }
};
...
function moveToFunds(alpha) {
	return function(d) {
		var centreY = payCentres[d.entity].y;
		var centreX = payCentres[d.entity].x;
		if (d.entity !== 'pub') {
			centreY = 300;
			centreX = 350;
		} else {
			centreX = payCentres[d.entity].x + 60;
			centreY = 380;
		}
		d.x += (centreX - d.x) * (brake + 0.02) * alpha * 1.1;
		d.y += (centreY - d.y) * (brake + 0.02) * alpha * 1.1;
...
$(document).ready(function() {
    ...
    return d3.csv("data/new7500up.csv", display);
